	...
	public Grass(float x, float y, float lenM, float disM, int dirN, int disN, float shrF) {
		...
		for(int d = 0; d < dirN; d++) {
			...
			for(int i = 1; i <= disN; i++) {
				...
				sX = x + Math2D.calcLenX(dis,dir) + Math2D.rnd(-1,1)*disM/disN;
				sY = y + Math2D.calcLenY(dis,dir) + Math2D.rnd(-1,1)*disM/disN;
				sZ = Heightmap.getInstance().getZ(sX,sY);
				
				sL = shrF*(disM + disM/disN - dis)/disM + (1-shrF);
				sL *= lenM*(.8f+Math2D.rnd(.4f));
