...
public class Body {
	...
	public void add(float x, float y, float z, float waistH, float dir) {
		float f, iF;
		f = frac;
		iF = 1-f;
		...
		GOGL.add3DBlock(-h/2,-h/2,0, h, h, bLen);
		...
		for(int i = -1; i <= 1; i += 2) {
			...
			GOGL.add3DBlock(0,-h/2,-h/2, w, h, h);
