...
public class ScriptBkp extends Action {
	private List<Task> taskList = new ArrayList<Task>();
	private List<Register> tempVars = new ArrayList<Register>();
	private List<Register> vars = new ArrayList<Register>();
	...
	public ScriptBkp(String name) {
		super(name, null,false);
	}
	public ScriptBkp(String name, Register[] parameterList, boolean hasOutput) {
		super(name, parameterList, hasOutput);
	}
	
	@Override
	public Register run(Register output, Register[] parameters) {
		if(parameters != null)
	...
	public static Register exec(String code) {
		StringExt2 codeExt = new StringExt2(code);
		cleanCode(codeExt);

		ScriptBkp tempScr = createScript(codeExt.get());
		Register v = tempScr.run(null, null);
		tempScr.destroy();
	...
	public static ScriptBkp createScript(String str) {
		Stack<ScriptBkp> scs = new Stack<ScriptBkp>();
		Stack<Register> os = new Stack<Register>();
	    ...
	    class Parser {
	        ...
	        void scpu(String name)		{sc = scs.push(new ScriptBkp(name));}
	        void scpo() {
	        	scs.pop();
	        	sc = scs.peek();
	        }

	        Register opu(Register v) 	{return o = os.push(v);}
	        void opo() {
	        	os.pop();
	        ...
	        Register parseStatements() {
	        	Register v = parseStatement();
	        ...
	        Register parseStatement() {
                eatSpace();
                if(nibble("if")) {
                	Register bool, skipIf = sc.addConstant(), skipElse = sc.addConstant();
	        ...
	        void parseParameters(List<Register> parameters) {
	        ...
	        Register parseOperation(Register v, String operation) {
	        	Register pv, dst;
	        	Task t;
	        	boolean eq = nibble('=');
	        ...
	        Register parseValue(final String sb, byte type) {
	        	if(type == P_NEW_OBJECT) {
	        		Register output = sc.addTrueVar(), outputVar, name = sc.addConstant(sb);
	        		...
	        		if(nibble(".$")) {
	                	...
	                	Register v = parseValue(s.toString(), P_VARIABLE);
	                	opo();
	                	return v;
	        		}
	        		else if(nibble("(")) {
	        			...
	        			Register[] argsA = new Register[args.size()];
	        			args.toArray(argsA);
	        		...
	        		if(o == null)
	        			return sc.addVar(Register.get(sc,sb, false,true));
	        		else {
	        			Register outputVar = sc.addTrueVar();
        			...
        			Register[] parArray = new Register[parameters.size()];
	...
	private Register addConstant(boolean value) {return addConstant().set(value);}
	private Register addConstant(double value) 	{return addConstant().set(value);}
	private Register addConstant(String value) 	{return addConstant().set(value);}
	private Register addConstant() 				{return addTempVar(true,false);}
	
	private Register addVar(Register v) {
		vars.add(v);
		return v;
	}
	private Register addTempVar(boolean isConstant, boolean isTrueVariable) {
		Register v = Register.get(this, "temp" + tempNum++, isConstant, isTrueVariable);
		tempVars.add(v);
		return v;
	}
	private Register addTrueVar() 				{return addTempVar(false,true);}
	private Register addTempBuffer() 			{return addTempVar(false,false);}
	
	private void removeTempVar(Register tempVar) {
		tempVar.destroy();
