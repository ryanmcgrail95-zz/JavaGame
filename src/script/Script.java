...
public class Script extends Action {
	...
	public Script(PMLMemory mem, String name, int regNum, int[] argInds, ConstantRegister[] constList,
			boolean hasOutput, Task[] taskList) {
	...
	/*
	 * public static Register getValue(String value) { boolean inName = false,
	 * period = false; int len = value.length(); String trueName = ""; char c;
	 * 
	 * for(int i = 0; i < len; i++) { c = value.charAt(i);
	 * 
	 * if(isWhiteSpace(c)) { if(inName) break; } else if(!inName) inName = true;
	 * 
	 * if(inName) trueName += c; }
	 * 
	 * Register val = null; val = Register.varMap.get(trueName);
	 * 
	 * if(val != null) return val; else if(period) return
	 * sc.addTemVar(true).set(Float.parseFloat(trueName)); else return
	 * sc.addTempVar(true).set(Integer.parseInt(trueName)); }
	 */
	...
	public static Register exec(PMLMemory mem, String code) throws ParseException {
		...
		Script tempScr = compile(mem, "__TEST__", code, varMap, null);
	...
	/*
	 * public static Register eval(final String str) { class Parser { int pos =
	 * -1, c;
	 * 
	 ...
	 * 
	 * Register parse() { eatChar(); Register v = parseExpression(); if (c !=
	 * -1) throw new RuntimeException("Unexpected: " + (char)c); return v; }
	 * 
	 * // Grammar: // expression = term | expression `+` term | expression `-`
	 * term // term = factor | term `*` factor | term `/` factor | term brackets
	 * // factor = brackets | number | factor `^` factor // brackets = `(`
	 * expression `)`
	 * 
	 * Register parseExpression() { Register v = parseTerm(); for (;;) {
	 * eatSpace(); if (c == '+') { // addition eatChar(); v.adde(parseTerm()); }
	 * else if (c == '-') { // subtraction eatChar(); v.sube(parseTerm()); }
	 * else { return v; } } }
	 * 
	 * Register parseTerm() { Register v = parseFactor(); for (;;) { eatSpace();
	 * if (c == '=') { eatChar(); if(!v.canBeSet()) throw new
	 * UnsupportedOperationException(); v.set(parseExpression()); v = v.copy();
	 * } else { if(v.canBeSet()) v = v.copy();
	 * 
	 * if (c == '/') { // division eatChar(); v.dive(parseFactor()); } else if
	 * (c == '*' || c == '(') { // multiplication if (c == '*') eatChar();
	 * v.multe(parseFactor()); } else { return v; } } } }
	 * 
	 * Register parseFactor() { Register v; boolean isVar = false, negate =
	 * false; eatSpace(); if (c == '+' || c == '-') { // unary plus & minus
	 * negate = c == '-'; eatChar(); eatSpace(); } if (c == '(') { // brackets
	 * eatChar(); v = parseExpression(); if (c == ')') eatChar(); } else { //
	 * numbers/variables StringBuilder sb = new StringBuilder();
	 * if(Character.isAlphabetic(c)) do { sb.append((char) c); eatChar(); }
	 * while(Character.isAlphabetic(c)); else while ((c >= '0' && c <= '9') || c
	 * == '.') { sb.append((char)c); eatChar(); } if (sb.length() == 0) throw
	 * new RuntimeException("Unexpected: " + (char)c);
	 * 
	 * v = parseValue(sb.toString()); } eatSpace(); if (c == '^') { //
	 * exponentiation if(v.canBeSet()) v = v.copy(); eatChar();
	 * v.powe(parseFactor()); } if (negate) { if(v.canBeSet()) v = v.copy();
	 * v.multe(-1); // unary minus is applied after exponentiation; e.g. -3^2=-9
	 * } return v; }
	 * 
	 * Register parseValue(final String sb) {
	 * if(Character.isAlphabetic(sb.charAt(0))) return Register.get(sb); else
	 * if(sb.contains(".")) return new Register().set(Double.parseDouble(sb));
	 * else return new Register().set(Integer.parseInt(sb)); } } return new
	 * Parser().parse(); }
	 */
	...
	public static Script compile(PMLMemory mem, String name, String code, NameIDMap<RegisterBlueprint> regIDMap,
		...
		class Parser {
			...
			ID<RegisterBlueprint> opu(ID<RegisterBlueprint> v) {
				return o = os.push(v);
			}
			...
			boolean ateComment = false;
			boolean didEatComment() {
				if(ateComment) {
					ateComment = false;
					return true;
				}
			...
			ID<RegisterBlueprint> parseStatements() throws ParseException {
				...
				for (;;) {
					// Log.println(Log.ID.PML, true, lc() + " ... " + c() + "
					// ... " + str);
...
//					//Log.println(Log.ID.PML, "c: \"" + c() + "\", \"" + lc() + "\"");
			...
			ID<RegisterBlueprint> addTask(String actionName, ID<RegisterBlueprint> outInd,
					ID<RegisterBlueprint>... parameterIndices) {
			...
			boolean isConstant(ID<RegisterBlueprint> i) {
				return i.get().get().isConstant();
			}

			boolean isTrueVariable(ID<RegisterBlueprint> i) {
				return i.get().get().isTrueVariable();
			}

			boolean isTempBuffer(ID<RegisterBlueprint> i) {
			...
			ID<RegisterBlueprint> parseExpression() throws ParseException {
				...
				for (;;) {
					...
					if (str.nibble('<')) {
						...
						pv = parseTerm();
						/*
						 * if(isConstant(pv) && isConstant(v)) { v.set(eq ?
						 * v.getNumber() <= pv.getNumber() : v.getNumber() <
						 * pv.getNumber()); sc.removeTempVar(pv); } else {
						 */
						...
						if (eq)
							...
							dst = addTempBuffer();
						// TODO: addTask(MiscAction.createCompareTask(dst, v,pv,
						// C_LESS_EQUAL));
						v = dst;
						// }
			...
			ID<RegisterBlueprint> parseOperation(boolean eq, ID<RegisterBlueprint> v, ID<RegisterBlueprint> pv,
					char operation) {
				 ...
				 * v.operation(operation+"=",pv); sc.removeTempVar(pv); } else {
				 */
				// dst = (!isConstant(v)) ? v : pv;
				// dst = eq ? v : (!isTempBuffer(dst) ? sc.addTempBuffer() :
				// dst);
			...
			ID<RegisterBlueprint> parseFactor() throws ParseException {
				...
				str.munchSpace();
				if ((n = str.nibbleMisc('+', '-')) != StringExt.NULL_CHAR) { // unary
																				// plus
																				// &
																				// minus
					negate = n == '-';
				...
				if (str.nibble('(')) { // brackets
					...
					if ((n = str.nibbleMisc('$', '#', '"', '@', '&')) != StringExt.NULL_CHAR
			...
			protected ID<RegisterBlueprint> setRegister(ID<RegisterBlueprint> regID, boolean value) {
				regID.get().get().set(value);
				return regID;
			}

			protected ID<RegisterBlueprint> setRegister(ID<RegisterBlueprint> regID, double value) {
				regID.get().get().set(value);
				return regID;
			}

			protected ID<RegisterBlueprint> setRegister(ID<RegisterBlueprint> regID, String value) {
				regID.get().get().set(value);
				return regID;
			}

			protected ID<RegisterBlueprint> setRegister(ID<RegisterBlueprint> regID, Object value) {
				regID.get().get().set(value);
				return regID;
			}

			ID<RegisterBlueprint> addConstant() {
				return addRegister(true, false);
			}

			ID<RegisterBlueprint> addConstant(boolean value) {
				return setRegister(addConstant(), value);
			}

			ID<RegisterBlueprint> addConstant(double value) {
				return setRegister(addConstant(), value);
			}

			ID<RegisterBlueprint> addConstant(String value) {
				return setRegister(addConstant(), value);
			}

			ID<RegisterBlueprint> addConstant(Object value) {
				return setRegister(addConstant(), value);
			}

			ID<RegisterBlueprint> addTrueVariable(String name) {
				return addRegister(name, false, true);
			}

			ID<RegisterBlueprint> addTempBuffer() {
			...
			ID<RegisterBlueprint> parseValue(final String sb, byte type) throws ParseException {
				...
				if (type == P_NEW_OBJECT) {
					...
					// TODO: add proper checking for whether or not output is
					// null
					...
					switch (sb) {
					case "TRUE":
						out = addConstant(true);
						break;
					case "FALSE":
						out = addConstant(false);
						break;
					case "PI":
						out = addConstant(Math.PI);
						break;
					case "TAU":
						out = addConstant(2 * Math.PI);
						break;
					case "E":
		...
		return s;
		/*
		 * System.out.println("Created new script: "); System.out.println(
		 * "\tTasks: " + sc.taskList.size()); for(Task t : sc.taskList)
